...
def _initialize_input_weights(args,edges_in,edges_out,edges_ex):
    input_weights = (2.0*np.random.random(size=(args.n*args.V*args.n_internal_units, args.n*args.V)) - 1.0)*args.input_scaling
    row = []
    col = []
    data = []
...
def sav_er1(preds2s,preds3s,error2s,error3s): 
    for ni in range(args.n):
        for vj in range(args.V):
            ...
            error_2pd.to_csv('error22m'+str(ni)+'_'+str(vj)+'_2.csv')
            error_3pd.to_csv('error22m'+str(ni)+'_'+str(vj)+'_3.csv')
    return()
...
def sav_er2(preds2s,preds3s,error2s,error3s):
    ...
    s2.to_csv('pred2s22m_2.csv')
    s3.to_csv('pred3s22m_3.csv')
